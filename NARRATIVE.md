...
**This is the shipped order**, 45 slides, matching `src/slides/index.js`
...
### ① Cold open — the flat screen, and the tube wakes at the first question
...
Still the glass, still head-on. The tube wakes on the first question, slide 7
(the title, the identity assets, the disclaimer, the chat window, Rob and the
code are the flat frames), so every beat here carries the same drive
...
| # | Slide | Glass | The argument |
|---|---|---|---|
| 7 | `q-stopping` | the question alone; Enter grows the chart, q1, **52%** often or daily | The survey borrowed the machine's own phrasing: *"just one more prompt."* Asked before it is answered (Scott, 2026-09-10): the headline sits on the glass on arrival, and Enter grows the five columns in and lands the 52%. |
| 8 | `quote-lunch` | the quote, large; a clock, and reminders at noon and six that get dismissed | **The overrun in a respondent's words** (Scott, 2026-09-10). They built the guardrail themselves and walk through it twice a day. Beside it, a day on a clock: the hands sweep, a "stop for lunch" card rises, waits, and is crossed off; at six it happens again. Read aloud, then move on — the machine explains it two slides later. |
...
| # | Slide | Glass | The argument |
|---|---|---|---|
| 14 | `q-pressure` | q3, **71%** · 37% daily | Seven in ten. Straight from the desk to the data: the camera pushes back into the glass and the office arrives behind it; the room does not see it yet. `cubicle-threshold` ("it is not just you.") and `productivity-paradox` (+26% PRs / −19% speed) were cut 2026-09-09 — the studies stay in `src/data/survey.js` for the talk to quote. |
| 15 | `quote-ship` | the quote, small and long; a week where one or two ships land, then a week where one stamps into every day | **The pressure, in respondents' words** (Scott, 2026-09-10) — three of them before the break that feels like theft. First the quota: a major ship every day, and punishment for anyone whose output did not rise. |
| 16 | `quote-architecture` | the quote; a grid of boxes wired up fast, its inside fading to ghost, a fault pulsing hot in the middle | **Knowing less about the system** while being asked for more of it — and the certainty that a problem is coming that nobody left will be able to reach. |
| 17 | `quote-fomo` | the quote; two gauges under one limit line — yours stops when the moon comes up, theirs fills to the line under a sun, the reset drops both | **The other side of the planet does not sleep.** Burning tokens to hit the usage limit before the reset, and blaming yourself for going to bed instead. The hyper-competition in one respondent's stomach. |
| 18 | `quote-breaks` | the quote, large; a mug of coffee inside a "no" sign that strokes itself on | **The pressure in a respondent's words** (Scott, 2026-09-10). Not a rule anyone wrote down — a break feels like theft. Beside it the steam keeps rising off the mug while the sign draws itself around it, holds, lets go and draws again. |
| 19 | `cubicle-wide` | (holds the breaks quote) | Neighbouring pools of agent activity over the partitions. If it's possible to go faster we must go faster — and nobody asks why. Saved effort returns as decisions, supervision and a higher baseline, never as rest. After the data, not before it (2026-09-02): the office is the pull-back that closes the act, the way the desk closes Pillar 1. |

### ④ Wall — the throttle

| # | Slide | Glass | The argument |
|---|---|---|---|
| 20 | `q-agents` | median **2**, 63% run 1–2 | Most people are not running ten agents. |
| 21 | `what-gets-pruned` | "grill me." — a brain on a grill | **The question, put to the room** (moved here from the phosphor exit, Scott 2026-09-10). Each time the brain lands a question pops up — *could you have written this without the model? did you read the diff? do you know why it works? could you debug it by hand? would you notice if it was wrong?* — Scott answers with **Y** or **N** and the answer flips it. Five questions, then the tally. The questions are placeholder copy. |
| 22 | `quote-doomscroll` | one respondent's words, large | **What it feels like from the inside** (Scott, 2026-09-10), before the data on how many machines they run: *"it feels like having doomscrolled for hours."* Verbatim, lowercase and all. Beside it, drawn in lines on its own clock: a phone whose feed never ends. |
| 23 | `quote-tired` | one respondent's words, large | *"I feel(!) like I'm more productive but have never felt so tired."* The whole talk in one line, from someone who took the survey. Beside it: the pile of things done grows a bar at a time while the battery drains and flickers at the bottom. |
| 24 | `agents-stopping` | **80%** (n=569) vs **40%** (n=2,253) | Double the overrun rate. More levers, more pulls. Exact: 453/569 = 79.6% of people running four or more agents (the scale tops out at "5+"), 890/2,253 = 39.5% of those running one or two — verified against the export's agents × q1 cross-tab 2026-09-02. `stopping-beats-count` (the same split inside the 1–2 agent band, 57% vs 21%) was cut 2026-09-10; the figures stay in survey.js to quote. |
| 25 | `agent-wall-near` | — | **After the data, not before it.** Neighbours enter peripheral vision. |
| 26 | `agent-wall` | — | The pull-back used to OPEN this act, so the room saw fifty-four screens and only then learned the median developer runs two — the reveal explaining itself away. Now the data lands first and the camera pulls back onto a wall they have every reason not to expect. The escalation is trivially logical and that is the problem: one, two, four — there is no number at which the reasoning stops. **Nothing may be scheduled between these two.** |
...
| # | Slide | Glass | The argument |
|---|---|---|---|
| 27 | `q-skills` | q4, **63%** diminishing | Only **12%** say sharpening (n=445). |
| 28 | `q-enjoyment` | q2, **57%** less | 18% the same. 24% enjoy it *more*. |
| 29 | `skills-enjoyment` | **68%** (n=2,279) vs **30%** (n=445) | **The finding.** ρ = **−0.37**, strongest negative in the survey. Not two findings — mostly the same people. Say plainly that the survey measured *belief* and never tested anyone; then say why belief is what matters, because it's what keeps you in the field. `agents-outcomes` (benefit and cost climbing together, skills flat and dashed) was cut 2026-09-10; the figures stay in survey.js. |
| 30 | `quote-spark-lost` | the quote, large; a flame dwindling to an ember while coins stack beside it | **The spark, lost** (Scott, 2026-09-10). What the enjoyment number feels like from inside one career: the passion gone, the money left, for as long as it lasts. The flame burns down over the cycle to an ember and a wisp of smoke, and a coin lands for every bit of it that goes. |
| 31 | `quote-spark` | the quote, large; a strike, a burst, a flame that catches and branches | **The spark, found** (Scott, 2026-09-10). The counterweight, in another respondent's words: not an accelerant but the spark itself, and things attempted that would never have been considered. A strike, a burst of hot rays, a flame that stays lit, and branches forking up out of it. The two sit back to back on purpose; the room hears both. |
| 32 | `quote-puzzle` | one respondent's words, large | **The finding, in a respondent's own words** (Scott, 2026-09-10): *"It's like solving a puzzle. You don't get any reward when another person solves the puzzle because you know that it was actually not you who solved it."* The reward was never the puzzle solved; it was solving it. Beside it, a jigsaw assembles itself piece by piece, flares when the last one lands, and clears to start again — somebody else is solving it. The last glass before the descent, so it occludes the phosphor swap. The first of the quote slides — one verbatim quote, alone, after the finding it speaks to. |
| 33 | `inside-glass` | the dream — fireflies | The whole descent in ONE move: the glass opens and the camera keeps going through it into the cloud. The motes are fireflies now (2026-09-02): each wanders on its own slow seeded path and blinks on its own rhythm, so the cloud is a swarm of live things rather than a drifting texture. `phosphor-approach` and `phosphor-threshold` used to hold in front of this as two separate stops, when the one thing a descent must not do is stop. |
| 34 | `synapse` | the fireflies settle into the network | **WHY 1 — NEUROPLASTICITY.** The brain is an efficient machine and prunes what it isn't using. The same motes that were wandering settle into their network positions and stay put — explained over it, which is not a metaphor for what's being described but a picture of it. The instrument you played in middle school. |
| 35 | `synapse-decay` | connections die one by one | **WHY 2 — use it or lose it**, watched rather than asserted. Say nothing. The silence is the point. |
...
|---|---|---|---|
| 36 | `return-cubicle` | `cubicle`, "what do we do" on the monitor | **One pull-back from inside the decayed synapse to this desk**, not stopping at the glass (Scott, 2026-09-10): the motes fade and the picture reforms on the way out, the set swaps behind the opaque glass, and the camera keeps going. Framed tight enough to read the line. **The ceiling.** Everything about to be said assumes you're allowed to stop. Against a quota, or a manager who treats the tool as a magic bullet, a 15-minute timer will not save you. Healthy prompting cannot be carried by personal discipline alone. The line stays on the monitor for the whole beat; `act-ceiling`, `early-career` and its caveat were cut 2026-09-02. |
| 37 | `quote-calmer` | the quote, large; five darting beads fold into one slow one | **What do we do — in respondents' words** (Scott, 2026-09-10). The first answer: fewer things at once. Beside it five tracks, each with a bead darting on its own rhythm, fold down into one line with one bead that drifts, and hold there. |
| 38 | `quote-one-project` | the quote, large; a goal drawn first, one frame, three sessions wired to it | **One project, planned** (Scott, 2026-09-10). The goal ring draws before anything else, then one frame, then three sessions light up inside it and run their lines up to the goal; the frames outside fade away. |
| 39 | `quote-outside` | the quote, large; the walk's path and bare line trees, coming past at walking pace | **GO OUTSIDE** (Scott, 2026-09-10). The walk slide became this quote; its path and trees stay on as the drawing, still moving on the free-running clock at an unhurried pace. `act-boundaries` ("go for a walk.") is kept in the catalog for `?visual` review. |
| 40 | `quote-phone` | the quote, large; a hammock between two trees, the phone buzzing on the ground | **Not checking** (Scott, 2026-09-10). A figure in a hammock slung between two of the walk's trees, swaying; the phone lies face down on the ground below, buzzes now and then, and stays where it is. |
| 41 | `quote-nine-to-five` | the quote, large; a clock hand sweeping nine to five and stopping | **Nine to five** (Scott, 2026-09-10). The hand sweeps from nine to five, lighting the arc it covers, and stops; the hours past five stay dark and the centre goes out. |
...
**The form itself** is no longer read out: `survey-questions` — the six
questions typed one per Enter behind a prompt — was cut 2026-09-10 (Scott), so
the code leads straight to the first chart. The wording stays in `FORM` and the
catalog.
