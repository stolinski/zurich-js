...
**This is the shipped order**, 46 slides, matching `src/slides/index.js`
...
|---|---|---|---|
| 37 | `return-cubicle` | `cubicle`, "what do we do" on the monitor | **One pull-back from inside the decayed synapse to this desk**, not stopping at the glass (Scott, 2026-09-10): the motes fade and the picture reforms on the way out, the set swaps behind the opaque glass, and the camera keeps going. Framed tight enough to read the line. **The ceiling.** Everything about to be said assumes you're allowed to stop. Against a quota, or a manager who treats the tool as a magic bullet, a 15-minute timer will not save you. Healthy prompting cannot be carried by personal discipline alone. The line stays on the monitor for the whole beat; `act-ceiling`, `early-career` and its caveat were cut 2026-09-02. |
| 38 | `quote-calmer` | the quote, large; five darting beads fold into one slow one | **What do we do — in respondents' words** (Scott, 2026-09-10). The first answer: fewer things at once. Beside it five tracks, each with a bead darting on its own rhythm, fold down into one line with one bead that drifts, and hold there. |
| 39 | `quote-one-project` | the quote, large; a goal drawn first, one frame, three sessions wired to it | **One project, planned** (Scott, 2026-09-10). The goal ring draws before anything else, then one frame, then three sessions light up inside it and run their lines up to the goal; the frames outside fade away. |
| 40 | `quote-outside` | the quote, large; the walk's path and bare line trees, coming past at walking pace | **GO OUTSIDE** (Scott, 2026-09-10). The walk slide became this quote; its path and trees stay on as the drawing, still moving on the free-running clock at an unhurried pace. `act-boundaries` ("go for a walk.") is kept in the catalog for `?visual` review. |
| 41 | `quote-phone` | the quote, large; a hammock between two trees, the phone buzzing on the ground | **Not checking** (Scott, 2026-09-10). A figure in a hammock slung between two of the walk's trees, swaying; the phone lies face down on the ground below, buzzes now and then, and stays where it is. |
| 42 | `quote-nine-to-five` | the quote, large; a clock hand sweeping nine to five and stopping | **Nine to five** (Scott, 2026-09-10). The hand sweeps from nine to five, lighting the arc it covers, and stops; the hours past five stay dark and the centre goes out. |
