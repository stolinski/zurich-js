...
**This is the shipped order**, 30 slides, matching `src/slides/index.js`
...
|---|---|---|---|
| 26 | `return-cubicle` | `cubicle`, "you have to be allowed / to stop." on the monitor | **One pull-back from inside the decayed synapse to this desk**, not stopping at the glass (Scott, 2026-09-10): the motes fade and the picture reforms on the way out, the set swaps behind the opaque glass, and the camera keeps going. Framed tight enough to read the line. **The ceiling.** Everything about to be said assumes you're allowed to stop. Against a quota, or a manager who treats the tool as a magic bullet, a 15-minute timer will not save you. Healthy prompting cannot be carried by personal discipline alone. The line stays on the monitor for the whole beat; `act-ceiling`, `early-career` and its caveat were cut 2026-09-02. |
| 27 | `act-boundaries` | `cubicle`, glass | "go for a walk." over a perspective line drawing of a path through bare line trees, walked along at walking pace. **How you'd know**, then **what works**, are spoken over the walk (`boundaries`, which carried them at the home desk under "you are in control.", was cut 2026-09-10): sleep first, loss of interest, exhaustion that doesn't clear, anxiety, pulling away from people; then somatization. Then: box the loop (10–15 min, then stop whether or not it landed — a timer beats a variable-reward schedule, willpower doesn't); make stopping structural, not moral; stay the one deciding (that's Pillar 2's answer — put the reps back); narrow the ambition; talk to people, not just the model. |
| 28 | `outro-qr` | the code alone, flat, at scanning size | **The door out** (Scott, 2026-09-10). The room has the whole argument; this is where it goes. Just the code — no count, no address (Scott, later the same day). Flat on purpose — the curve, mask and beam are what made the code hard to scan — so the glass is a screen recording again for one beat, and the close wakes the tube a last time on the way out. |
| 29 | `close` | `home`, lights off; autoplay | The lights go off in the room over the flight and the title comes back on the glass — same words as slide 0, on a screen you now know is an object, alone in the dark the way it was at 3am. Then, on the machine's clock (Scott, 2026-09-10): a few seconds' hold, the tube shuts off — the picture collapses to a line, the line to a dot — a moment of real dark, and **"Thank you"** types itself on the dead glass. Stop talking before the tube does. `three-r` was cut 2026-09-02. |
