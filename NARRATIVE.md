...
**This is the shipped order**, 41 slides, matching `src/slides/index.js`
...
| 16 | `quote-breaks` | the quote, large; a mug of coffee inside a "no" sign that strokes itself on | **The pressure in a respondent's words** (Scott, 2026-09-10). Not a rule anyone wrote down — a break feels like theft. Beside it the steam keeps rising off the mug while the sign draws itself around it, holds, lets go and draws again. |
| 17 | `cubicle-wide` | (holds the breaks quote) | Neighbouring pools of agent activity over the partitions. If it's possible to go faster we must go faster — and nobody asks why. Saved effort returns as decisions, supervision and a higher baseline, never as rest. After the data, not before it (2026-09-02): the office is the pull-back that closes the act, the way the desk closes Pillar 1. |

### ④ Wall — the throttle

| # | Slide | Glass | The argument |
|---|---|---|---|
| 18 | `q-agents` | median **2**, 63% run 1–2 | Most people are not running ten agents. |
| 19 | `what-gets-pruned` | "grill me." — a brain on a grill | **The question, put to the room** (moved here from the phosphor exit, Scott 2026-09-10). Each time the brain lands a question pops up — *could you have written this without the model? did you read the diff? do you know why it works? could you debug it by hand? would you notice if it was wrong?* — Scott answers with **Y** or **N** and the answer flips it. Five questions, then the tally. The questions are placeholder copy. |
| 20 | `quote-doomscroll` | one respondent's words, large | **What it feels like from the inside** (Scott, 2026-09-10), before the data on how many machines they run: *"it feels like having doomscrolled for hours."* Verbatim, lowercase and all. Beside it, drawn in lines on its own clock: a phone whose feed never ends. |
| 21 | `quote-tired` | one respondent's words, large | *"I feel(!) like I'm more productive but have never felt so tired."* The whole talk in one line, from someone who took the survey. Beside it: the pile of things done grows a bar at a time while the battery drains and flickers at the bottom. |
| 22 | `agents-stopping` | **80%** (n=569) vs **40%** (n=2,253) | Double the overrun rate. More levers, more pulls. Exact: 453/569 = 79.6% of people running four or more agents (the scale tops out at "5+"), 890/2,253 = 39.5% of those running one or two — verified against the export's agents × q1 cross-tab 2026-09-02. `stopping-beats-count` (the same split inside the 1–2 agent band, 57% vs 21%) was cut 2026-09-10; the figures stay in survey.js to quote. |
| 23 | `agent-wall-near` | — | **After the data, not before it.** Neighbours enter peripheral vision. |
| 24 | `agent-wall` | — | The pull-back used to OPEN this act, so the room saw fifty-four screens and only then learned the median developer runs two — the reveal explaining itself away. Now the data lands first and the camera pulls back onto a wall they have every reason not to expect. The escalation is trivially logical and that is the problem: one, two, four — there is no number at which the reasoning stops. **Nothing may be scheduled between these two.** |
...
|---|---|---|---|
| 32 | `return-cubicle` | `cubicle`, "what do we do" on the monitor | **One pull-back from inside the decayed synapse to this desk**, not stopping at the glass (Scott, 2026-09-10): the motes fade and the picture reforms on the way out, the set swaps behind the opaque glass, and the camera keeps going. Framed tight enough to read the line. **The ceiling.** Everything about to be said assumes you're allowed to stop. Against a quota, or a manager who treats the tool as a magic bullet, a 15-minute timer will not save you. Healthy prompting cannot be carried by personal discipline alone. The line stays on the monitor for the whole beat; `act-ceiling`, `early-career` and its caveat were cut 2026-09-02. |
| 33 | `quote-calmer` | the quote, large; five darting beads fold into one slow one | **What do we do — in respondents' words** (Scott, 2026-09-10). The first answer: fewer things at once. Beside it five tracks, each with a bead darting on its own rhythm, fold down into one line with one bead that drifts, and hold there. |
| 34 | `quote-one-project` | the quote, large; a goal drawn first, one frame, three sessions wired to it | **One project, planned** (Scott, 2026-09-10). The goal ring draws before anything else, then one frame, then three sessions light up inside it and run their lines up to the goal; the frames outside fade away. |
| 35 | `quote-outside` | the quote, large; the walk's path and bare line trees, coming past at walking pace | **GO OUTSIDE** (Scott, 2026-09-10). The walk slide became this quote; its path and trees stay on as the drawing, still moving on the free-running clock at an unhurried pace. `act-boundaries` ("go for a walk.") is kept in the catalog for `?visual` review. |
| 36 | `quote-phone` | the quote, large; a hammock between two trees, the phone buzzing on the ground | **Not checking** (Scott, 2026-09-10). A figure in a hammock slung between two of the walk's trees, swaying; the phone lies face down on the ground below, buzzes now and then, and stays where it is. |
| 37 | `quote-nine-to-five` | the quote, large; a clock hand sweeping nine to five and stopping | **Nine to five** (Scott, 2026-09-10). The hand sweeps from nine to five, lighting the arc it covers, and stops; the hours past five stay dark and the centre goes out. |
