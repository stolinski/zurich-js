...
| 26 | `act-boundaries` | `cubicle`, glass | "go for a walk." over a perspective line drawing of a path through bare line trees, walked along at walking pace. |
...
| 28 | `outro-qr` | the code again, flat, at scanning size | **The door out** (Scott, 2026-09-10). The room has the whole argument; this is where it goes. Flat on purpose — the curve, mask and beam are what made the code hard to scan — so the glass is a screen recording again for one beat, and the close wakes the tube a last time on the way out. |
| 29 | `close` | `home`, lights off | The lights go off in the room over the flight and the title comes back on the glass — same words as slide 0, on a screen you now know is an object, alone in the dark the way it was at 3am. Then the idle cursor. Hold it blinking and stop talking. `three-r` was cut 2026-09-02. |
