...
**This is the shipped order**, 32 slides, matching `src/slides/index.js`
...
| 15 | `what-gets-pruned` | "grill me." — a brain on a grill | **The question, put to the room** (moved here from the phosphor exit, Scott 2026-09-10). Each time the brain lands a question pops up — *could you have written this without the model? did you read the diff? do you know why it works? could you debug it by hand? would you notice if it was wrong?* — Scott answers with **Y** or **N** and the answer flips it. Five questions, then the tally. The questions are placeholder copy. |
| 16 | `quote-doomscroll` | one respondent's words, large | **What it feels like from the inside** (Scott, 2026-09-10), before the data on how many machines they run: *"it feels like having doomscrolled for hours."* Verbatim, lowercase and all. Beside it, drawn in lines on its own clock: a phone whose feed never ends. |
| 17 | `quote-tired` | one respondent's words, large | *"I feel(!) like I'm more productive but have never felt so tired."* The whole talk in one line, from someone who took the survey. Beside it: the pile of things done grows a bar at a time while the battery drains and flickers at the bottom. |
| 18 | `agents-stopping` | **80%** (n=569) vs **40%** (n=2,253) | Double the overrun rate. More levers, more pulls. Exact: 453/569 = 79.6% of people running four or more agents (the scale tops out at "5+"), 890/2,253 = 39.5% of those running one or two — verified against the export's agents × q1 cross-tab 2026-09-02. `stopping-beats-count` (the same split inside the 1–2 agent band, 57% vs 21%) was cut 2026-09-10; the figures stay in survey.js to quote. |
...
|---|---|---|---|
| 28 | `return-cubicle` | `cubicle`, "you have to be allowed / to stop." on the monitor | **One pull-back from inside the decayed synapse to this desk**, not stopping at the glass (Scott, 2026-09-10): the motes fade and the picture reforms on the way out, the set swaps behind the opaque glass, and the camera keeps going. Framed tight enough to read the line. **The ceiling.** Everything about to be said assumes you're allowed to stop. Against a quota, or a manager who treats the tool as a magic bullet, a 15-minute timer will not save you. Healthy prompting cannot be carried by personal discipline alone. The line stays on the monitor for the whole beat; `act-ceiling`, `early-career` and its caveat were cut 2026-09-02. |
| 29 | `act-boundaries` | `cubicle`, glass | "go for a walk." over a perspective line drawing of a path through bare line trees, walked along at walking pace. **How you'd know**, then **what works**, are spoken over the walk (`boundaries`, which carried them at the home desk under "you are in control.", was cut 2026-09-10): sleep first, loss of interest, exhaustion that doesn't clear, anxiety, pulling away from people; then somatization. Then: box the loop (10–15 min, then stop whether or not it landed — a timer beats a variable-reward schedule, willpower doesn't); make stopping structural, not moral; stay the one deciding (that's Pillar 2's answer — put the reps back); narrow the ambition; talk to people, not just the model. |
| 30 | `outro-qr` | the code alone, flat, at scanning size | **The door out** (Scott, 2026-09-10). The room has the whole argument; this is where it goes. Just the code — no count, no address (Scott, later the same day). Flat on purpose — the curve, mask and beam are what made the code hard to scan — so the glass is a screen recording again for one beat, and the close wakes the tube a last time on the way out. |
| 31 | `close` | `home`, lights off; autoplay | The lights go off in the room over the flight and the title comes back on the glass — same words as slide 0, on a screen you now know is an object, alone in the dark the way it was at 3am. Then, on the machine's clock (Scott, 2026-09-10): a few seconds' hold, the tube shuts off — the picture collapses to a line, the line to a dot — a moment of real dark, and **"Thank you"** types itself on the dead glass. Stop talking before the tube does. `three-r` was cut 2026-09-02. |
